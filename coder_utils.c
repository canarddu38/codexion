...
/*   Updated: 2026/03/03 18:33:36 by julcleme         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
bool	can_compile(t_coder_config *me)
{
	...
	first_dongle[0] = me->id - 1;
	second_dongle[0] = me->id % me->program_args.nb_coders;
	current_time = log_message(0, -1);
	if (is_coder_busy(me, first_dongle[0], second_dongle[0], current_time))
		return (false);
	if (me->program_args.scheduler == SCHEDULER_EDF)
	{
		...
		while (current)
		{
			first_dongle[1] = current->coder->id - 1;
			second_dongle[1] = current->coder->id % me->program_args.nb_coders;
			if (first_dongle[0] == second_dongle[1]
				|| second_dongle[0] == first_dongle[1])
				break ;
			current = current->next;
		}
		return (current && me->id == current->coder->id);
	}
	return (true);
}
